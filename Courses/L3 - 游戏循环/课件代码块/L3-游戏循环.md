...
| 块 | 内容 | PPT 用途 |
|----|------|----------|
| 块1 | gem.gd 动画 animate_clear + Tween 速查 | 模板 T3：动画串行 |
...
## 块1 · Tween 组件 + animate_clear（gem.gd ④）——模板 T3

```gdscript
## 动画：消除（缩小到 0.2 倍 + 淡出到透明，0.2 秒，两个效果同时进行）
## gem.gd 唯一的动画函数（2026-09-09：animate_swap_to / animate_fall_to 已删）
...
**Tween 组件速查（配合拆页文案 A6 页）：**

| 组件 | 作用 |
|---|---|
| `create_tween()` | 开一个动画调度器 |
| `tween_property(目标, "属性", 终点, 时长)` | 让属性平滑变化（补间） |
| `set_parallel(true)` | 本组属性同时播（内部并行） |
| `await tween.finished` | 等动画播完再继续（T3 模板） |
...
- **⚠️ 代码现状（2026-09-09）**：gem.gd 只有 `animate_clear` 一个动画函数；交换动画 = `board._animate_swap_pair`（L2），下落动画 = `fall_and_refill` 的 movers 统一 Tween——分工：**gem 提供能力、board 统一调度**（animate_swap_to / animate_fall_to 已删：死代码，完整版也没有）
